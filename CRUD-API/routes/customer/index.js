...
customerRouter.put('/update-customer', (req, res, next) => {
   ...
   if (query.update == undefined || query.filter == undefined || !isValidIncommingCustomerQuery(query.filter)) {
      res.status(400).send(
   ...
   updateCustomer(query)
      ...
      .catch((err) => {
         ...
         res.status(500).send(
            'Server Error. Route update-customer. Query call.'
         );
      });
});
