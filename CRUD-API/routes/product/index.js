...
productRouter.post('/create-product', (req, res, next) => {
   ...
   try {
      product = new Product(
         ...
         query.sellAtList == 'false' || query.sellAtList == undefined? false : true, 
         query.productName,
         query.productName, 
         query.productDescription
      );
      console.debug('create-poduct: product received. name: ${product.productName} price: ${product.price}')
   } catch (error) {
      console.debug('Invalid query sent. Unable to create product instance.');
      res.status(400).send('Unable to create instance of Product. Invalid product.');
   }
