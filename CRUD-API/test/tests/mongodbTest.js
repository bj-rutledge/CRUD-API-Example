...
test('Test insert order and product.', async () => {
    ...
    */
   for (let i = 0, productNum = 7777, sellAtList = true, cost = 35, listPrice = 40; i < numProducts; i++, productNum++, sellAtList = !sellAtList, cost += 5, listPrice += 5) {
   ...
   const order = new types.Order(new Date(Date.now()), orderNum, products, customerNum);
   ...
   for(let i = 0; i < products.length; i++){
      const productRes = await ProductModel.find({productNumber: products[i].number}).exec();
